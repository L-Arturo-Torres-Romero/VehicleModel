function loadInterface()
    load('waypoints.mat');
    % Check if the variable 'racetrackwaypoints' exists in the workspace
 %   if evalin('base', 'exist(''racetrackwaypoints'', ''var'')')
        % Retrieve the 'racetrackwaypoints' variable from the workspace
 %       racetrackwaypoints = evalin('base', 'racetrackwaypoints');
   ...
   % else
  %      error('The variable "racetrackwaypoints" does not exist in the workspace.');
 %   end
end
...
function acceptCallback(fig, txt1, txt2, txt3)
   ...
   VehicleObj = Simulink.data.dictionary.open('VehicleData.sldd');
